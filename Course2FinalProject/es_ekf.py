...
p_est[0] = gt.p[0]
v_est[0] = gt.v[0]
q_est[0] = Quaternion(axis_angle=gt.r[0]).to_numpy()
p_cov[0] = np.eye(9)  # covariance of estimate
gnss_i = 0
lidar_i = 0


p_check = np.zeros((3,1))
v_check = np.zeros((3,1))
q_check = np.zeros((4,1))
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    Kk =  p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T+ np.identity(3) * sensor_var )
    ...
    q_check = Quaternion(euler=dxk[6:9]).normalize().quat_mult(Quaternion(*q_check))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    q_check = Quaternion(axis_angle = delta_t * imu_w.data[k-1]).normalize().quat_mult(q_est[k-1])
    q_check = Quaternion(*q_check).normalize().to_numpy()
    # 1.1 Linearize Motion Model
     
    Fk = np.eye(9) #1
    Fk[0:3,3:6] = np.eye(3) * delta_t #2
    Fk[3:6,6:9] = -skew_symmetric(C_ns.dot(imu_f.data[k-1])) * delta_t #3print(Fk)
    
    Qk = np.zeros((6,6))
    Qk[0:3,0:3] = (delta_t ** 2) *var_imu_f *np.identity(3)
    Qk[3:6,3:6] = (delta_t ** 2) * var_imu_w *np.identity(3)
    # 2. Propagate uncertainty
    p_cov_check = Fk @ p_cov [k-1] @ Fk.T + l_jac @ Qk @ l_jac.T
    # 3. Check availability of GNSS and LIDAR measurements
    #gnss_i = gnss.data.shape[0]
    #print(gnss.data.shape)
    for gk in range(0, gnss.data.shape[0]):
        if (gnss.t[gk] == imu_f.t[k] ):
            gnss_i = gk
            print("gnss_i = ",gnss_i)
            (p_check, v_check, q_check, p_cov_check) = measurement_update(var_gnss,p_cov_check, gnss.data[gnss_i], p_check, v_check, q_check)
    for lk in range(0, lidar.data.shape[0]):
        if (lidar.t[lk] == imu_f.t[k] ):
            lidar_i = lk
            print("lidar_i = ",lidar_i)
